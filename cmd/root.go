...
var (
	rootCmd = &cobra.Command{
		Use:   "gocat",
		Short: "Combine multiple go source files into a single source file",
		Long: `gocat combines multitple go source files into a single source file.
You can think of it as a kind of "webpack" for go files.

Adopted from other 
gocat implementations before, my primary use case is to take a more complex
project and simplify it for submission to coding exercises that require a
single file to be submitted
...
func cat(_ *cobra.Command, args []string) {
	...
	for _, name := range args {
		if viper.GetBool("ignore-tests") && strings.HasSuffix(name, "_test.go") {
			continue
		}
	...
	rename := func(id *ast.Ident) {
		if !renamed[id] && id != nil && id.Name != "_" && id.Name != "" {
			id.Name = viper.GetString("prefix") + id.Name
			renamed[id] = true
		}
	}
	...
	for _, f := range files {
		walk(f, func(n interface{}) {
			...
			if ok {
				if id, ok := kv.Key.(*ast.Ident); ok && renamed[id] && isType[id.Name[len(viper.GetString("prefix") ):]] {
					id.Name = id.Name[len(viper.GetString("prefix")):]
				}
			}
			id, ok := n.(*ast.Ident)
	...
	for i, name := range names {
		...
		if i == 0 {
			...
			if viper.GetString("package")  != "" {
				f.Name.Name = viper.GetString("package")
			}
		} else {
			f.Name = &ast.Ident{Name: "PACKAGE-DELETE-ME"}
			for _, spec := range f.Imports {
				fpath, err := strconv.Unquote(spec.Path.Value)
				if err != nil {
